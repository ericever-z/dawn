...
;define(function(){
    ...
    DragResize.prototype.updateMin = function(config){
        for(var p in props){
           this[p] = (typeof config[p] == 'undefined') ? props[p] : config[p]; 
        } 
    };
    ...
    DragResize.prototype.mouseMove = function(e) { with (this)
    {
     ...
     //element.style.width = Math.min(elmW,window.getComputedStyle(element).width) + "px";      
//     console.log(": " + elmW + " // " + window.getComputedStyle(element).width  + " //  " + window.getComputedStyle(element.parentNode).width); 
    ...
    DragResize.prototype.resizeHandleSet = function(elm, show) { with (this)
    {
     ...
     if (!elm._handle_tr)
     {
      ...
      {
       // Create 4 news spans, assign each a generic + specific class.
       var hspan = document.createElement('span');
       hspan.className = myName + ' ' +  myName + '-' + handles[h];
       elm['_handle_' + handles[h]] = elm.appendChild(hspan);
      }
     }
